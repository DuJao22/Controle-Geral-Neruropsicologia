...
def get_db_connection():
    ...
    try:
        ...
        if 'localizacao' not in columns:
            conn.execute('ALTER TABLE pacientes ADD COLUMN localizacao TEXT DEFAULT "Belo Horizonte"')
            conn.commit()
...
def init_db():
    ...
    tables = [
        '''CREATE TABLE IF NOT EXISTS medicos (
            ...
            localizacao TEXT DEFAULT 'Belo Horizonte',
    ...
    cursor = conn.execute('SELECT * FROM medicos WHERE email = ?',
                          ('admin@admin.com', ))
    ...
    if not admin:
        ...
        conn.execute(
            ...
            VALUES (?, ?, ?, ?)
        ''', ('Administrador', 'admin@gmail.com', senha_hash, 'ADMIN'))
        conn.commit()
...
def login():
    if request.method == 'POST':
        ...
        conn = get_db_connection()
        cursor = conn.execute(
            'SELECT * FROM medicos WHERE email = ? AND ativo = 1', (email, ))
...
def medico_dashboard():
    ...
    for paciente in pacientes:
        data_limite = datetime.strptime(paciente.data_limite,
                                        '%Y-%m-%d').date()
...
def medico_configuracoes():
    ...
    if request.method == 'POST':
        ...
        conn = get_db_connection()
        cursor = conn.execute('SELECT senha FROM medicos WHERE id = ?',
                              (session['user_id'], ))
        ...
        conn.execute('UPDATE medicos SET senha = ? WHERE id = ?',
...
def reset_password(medico_id):
    ...
    try:
        ...
        return jsonify({
            ...
            True,
            'message':
            f'Senha resetada com sucesso. Nova senha: {nova_senha}'
        })
    except Exception as e:
        return jsonify({
            'success': False,
...
@app.route('/admin/toggle_status/<int:medico_id>/<int:status>',
           methods=['POST'])
def toggle_status(medico_id, status):
    ...
    try:
        conn = get_db_connection()
        conn.execute('UPDATE medicos SET ativo = ? WHERE id = ?',
                     (status, medico_id))
        conn.commit()
        conn.close()
...
def novo_paciente():
    ...
    if request.method == 'POST':
        ...
        localizacao = request.form.get('localizacao', 'Belo Horizonte')  # Default to BH
        data_inicio = datetime.strptime(request.form['data_inicio'],
                                        '%Y-%m-%d').date()
        ...
        cursor = conn.execute('SELECT id FROM pacientes WHERE cpf = ?', (cpf,))
        ...
        try:
            conn.execute(
                '''
                INSERT INTO pacientes (nome, cpf, carteirinha, localizacao, data_inicio, data_limite, medico_id)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (nome, cpf, carteirinha, localizacao, data_inicio, data_limite, medico_id))
            conn.commit()
            conn.close()
            ...
            conn.close()
            flash('Erro ao cadastrar paciente. CPF pode já estar em uso.', 'error')
            return render_template('novo_paciente.html')
...
def paciente_detalhes(paciente_id):
    ...
    cursor = conn.execute(
        '''
        SELECT p.id, p.nome, p.cpf, p.carteirinha, p.data_inicio, p.data_limite, p.medico_id, p.status, p.localizacao, m.nome as medico_nome
        FROM pacientes p
        JOIN medicos m ON p.medico_id = m.id
        WHERE p.id = ?
    ''', (paciente_id, ))
    ...
    paciente.localizacao = paciente_raw[8]
    paciente.medico_nome = paciente_raw[9]

    # Check permissions
    if not session.get(
            'is_admin') and paciente.medico_id != session['user_id']:
...
def adicionar_sessao():
    ...
    # Check permissions
    cursor = conn.execute('SELECT medico_id FROM pacientes WHERE id = ?',
                          (paciente_id, ))
...
def adicionar_senha():
    ...
    # Check permissions
    cursor = conn.execute('SELECT * FROM pacientes WHERE id = ?',
                          (paciente_id, ))
    paciente = cursor.fetchone()
    if not session.get('is_admin') and paciente[6] != session[
            'user_id']:  # paciente[6] = medico_id
        flash('Acesso negado', 'error')
        conn.close()
        return redirect(url_for('medico_dashboard'))
    ...
    if existing:
        flash(f'Senha do tipo {tipo} já foi cadastrada para este paciente',
              'error')
    else:
...
def upload_laudo():
    ...
    if sessoes_count < 8 and not finalizar:
        flash(
            'Laudo só pode ser enviado após 8 sessões ou marcando "Finalizar paciente"',
            'error')
        conn.close()
        return redirect(url_for('paciente_detalhes', paciente_id=paciente_id))
    ...
    file_extension = os.path.splitext(
        original_filename)[1] if '.' in original_filename else ''
    filename = secure_filename(
        f"laudo_{paciente_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}{file_extension}"
    )
...
@app.route('/download_laudo/')
@app.route('/download_laudo/<filename>')
def download_laudo(filename=None):
    if 'user_id' not in session:
        return redirect(url_for('login'))
    ...
    try:
        # Verify the file belongs to a patient the user can access
        cursor = conn.execute(
            '''
            SELECT l.paciente_id, p.medico_id 
            FROM laudos l
            JOIN pacientes p ON l.paciente_id = p.id
            WHERE l.caminho_arquivo = ?
        ''', (filename, ))

        file_info = cursor.fetchone()

        if not file_info:
            flash('Arquivo não encontrado no sistema', 'error')
            conn.close()
            return redirect(url_for('medico_dashboard'))

        # Check permissions - doctors can only access their patients' files
        if not session.get('is_admin') and file_info[1] != session['user_id']:
            flash('Acesso negado', 'error')
            conn.close()
            return redirect(url_for('medico_dashboard'))
        ...
        if filename.startswith('laudo_'):
            # Try to extract a more user-friendly name
            parts = filename.split('_')
            if len(parts) >= 3:
                # Keep the file extension
                extension = os.path.splitext(filename)[1]
                original_name = f"Laudo_Paciente_{parts[1]}{extension}"
...
def relatorio_medico():
    ...
    sessoes_por_mes = [{
        'mes': row[0],
        'total': row[1]
    } for row in sessoes_por_mes_raw]
...
def paciente_login():
    """Login para pacientes usando CPF"""
    if request.method == 'POST':
        cpf = request.form['cpf']
        
        # Normalizar CPF para busca - remover todos os caracteres não numéricos
        cpf_limpo = ''.join(filter(str.isdigit, cpf))
        
        conn = get_db_connection()
        # Buscar comparando apenas os dígitos do CPF (pacientes ativos ou finalizados podem acessar)
        cursor = conn.execute(
            '''SELECT id, nome, cpf, status FROM pacientes 
        ...
        if paciente:
            ...
            session['paciente_nome'] = paciente[1]
            session['paciente_cpf'] = paciente[2]
            session['is_paciente'] = True
            return redirect(url_for('paciente_area'))
        else:
            flash('CPF não encontrado no sistema', 'error')
    
    return render_template('paciente_login.html')


@app.route('/paciente/area')
def paciente_area():
    """Área do paciente com seus laudos disponíveis"""
    if not session.get('is_paciente'):
    ...
    cursor = conn.execute(
        '''
        SELECT p.id, p.nome, p.cpf, p.data_inicio, p.data_limite, p.status, p.localizacao, m.nome as medico_nome
        FROM pacientes p
        JOIN medicos m ON p.medico_id = m.id
        WHERE p.id = ?
        ''',
    ...
    paciente.data_inicio = paciente_raw[3]
    paciente.data_limite = paciente_raw[4]
    paciente.status = paciente_raw[5]
    paciente.localizacao = paciente_raw[6]
    paciente.medico_nome = paciente_raw[7]
    ...
    laudos_raw = cursor.fetchall()
    
    laudos = []
    for row in laudos_raw:
        laudo = type('Laudo', (), {})()
        laudo.id = row[0]
        laudo.nome_arquivo = row[1].split('/')[-1]  # Só o nome do arquivo
        laudo.data_upload = row[2]
        laudos.append(laudo)
    
    # Sessões realizadas
    cursor = conn.execute(
        'SELECT COUNT(*) FROM sessoes WHERE paciente_id = ?',
        (paciente_id,)
    )
    ...
    return render_template('paciente_area.html', 
                         paciente=paciente,
                         laudos=laudos,
...
@app.route('/paciente/download/<int:laudo_id>')
def paciente_download_laudo(laudo_id):
    """Download de laudo pelo paciente"""
    if not session.get('is_paciente'):
        return redirect(url_for('paciente_login'))
    ...
    conn = get_db_connection()
    cursor = conn.execute(
        'SELECT caminho_arquivo FROM laudos WHERE id = ? AND paciente_id = ?',
    ...
    if not laudo:
        flash('Laudo não encontrado ou acesso negado', 'error')
        return redirect(url_for('paciente_area'))
    
    caminho_arquivo = laudo[0]
    
    # Extrair o nome do arquivo de forma mais robusta
    if '/' in caminho_arquivo:
        nome_arquivo = caminho_arquivo.split('/')[-1]
    elif '\\' in caminho_arquivo:
        nome_arquivo = caminho_arquivo.split('\\')[-1]
    else:
        nome_arquivo = caminho_arquivo
    
    # Verificar se o arquivo existe no servidor
    full_path = os.path.join(app.config['UPLOAD_FOLDER'], nome_arquivo)
...
def paciente_logout():
    """Logout do paciente"""
    # Limpar apenas dados do paciente da sessão
    if 'paciente_id' in session:
        del session['paciente_id']
    if 'paciente_nome' in session:
        del session['paciente_nome']
    if 'paciente_cpf' in session:
        del session['paciente_cpf']
    if 'is_paciente' in session:
        del session['is_paciente']
